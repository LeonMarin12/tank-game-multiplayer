...
func _ready() -> void:
	...
	if multiplayer.is_server():
		...
		# lado. Es el servidor quien decide cuando spawnearnos (ver
		# notify_ready) — no al reves. Ver esa funcion para el porque.
...
#
# NOTA: se probo primero controlar esto con visibilidad de MultiplayerSynchronizer
# (public_visibility=false + reveal_to() a mano) en vez de esperar confirmacion
# de todos — resulto no funcionar de forma confiable, porque la autoridad del
# MultiplayerSynchronizer de cada Player es el peer DUEÑO del tanque (necesario
# para que el pueda transmitir su propio movimiento), no el servidor, y Godot
# no deja que alguien que no es la autoridad de un synchronizer controle su
# visibilidad de forma confiable (es una limitacion conocida y documentada del
# motor, no un bug nuestro). Por eso se volvio a este esquema, mas simple y sin
# esa dependencia: esperar a que todos confirmen antes de spawnear.
...
# congelado en el origen durante toda la partida, no solo un frame perdido).
...
# depender de eso a ciegas y dejarla en el origen mientras tanto. La autoridad
# de cada copia la resuelve player_controller.gd (_enter_tree) por el nombre
# del nodo — no importa que ESTE peer sea quien hizo el add_child.
@rpc("authority", "reliable")
